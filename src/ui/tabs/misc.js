...
function miscFields(state, item = {}, defaultDate = '') {
  return [
    field('日期', dateSelect('date', item.date || defaultDate)),
...
export function renderMisc(state, actions) {
  const editor = actions.editing?.type === 'misc'
    ...
    : null;
  const rows = getMiscellaneous(state, { limit: 50, clone: false })
    .filter((row) => row.key !== 'career_start' && row.key !== 'career_opening')
    .map((row) => h('li', { class: 'fcl-list-row' }, [
  ...
  return h('div', {}, [
    editor ? renderRecordForm('编辑杂项', miscFields(state, editor, eventDate), '保存杂项', async (data) => {
      await actions.save((draft) => updateMiscellaneous(draft, editor.id, miscPayload(data)));
      actions.clearEditing();
    }) : renderRecordForm('新增杂项', miscFields(state, {}, eventDate), '新增杂项', async (data, form) => {
